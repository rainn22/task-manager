...
export default function TaskPage() {
  ...
  const isError = tasksQuery.isError || projectsQuery.isError || membersQuery.isError;

  const error = tasksQuery.error || projectsQuery.error || membersQuery.error;
  ...
  if (isLoading) {
    return (
      <div className="p-8">
        <h1 className="text-4xl font-bold text-zinc-900 dark:text-zinc-100">Tasks</h1>
  ...
  if (isError) {
    return (
      <div className="p-8">
        <h1 className="text-4xl font-bold text-red-700 dark:text-red-400">Error</h1>
        <p className="text-red-500 mt-2">{(error as Error).message || 'Failed to load tasks.'}</p>
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold text-zinc-900 dark:text-zinc-100 mb-1">Tasks</h1>
      ...
      <Card className="rounded-lg shadow p-4 bg-white dark:bg-zinc-900">
        <ul>
          {filteredTasks.map((task) => {
            ...
            const project =
              projectNameById.get(task.projectId ?? '') ?? getProjectName(projects, task.projectId);
            const comments = task.comments?.length ?? 0;
            const attachments = task.attachments?.length ?? 0;
            const flagged = task.priority === 'high';
            const due = formatDueDate(task.dueDate);
            const initialsList = getAssigneeInitialsList(members, task.assignees ?? []);
            ...
            return (
              <TaskRow
                ...
                id={task.id}
                checked={checked}
                onCheckedChange={() => mutation.mutate({ taskId: task.id, checked: !checked })}
                title={task.title}
